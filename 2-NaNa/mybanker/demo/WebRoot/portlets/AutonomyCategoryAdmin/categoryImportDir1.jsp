...
<p>
	<%=rb.getString("categoryImportDir1.importInfo")%>
</p>
<p>
	<%=rb.getString("categoryImportDir1.dirStruc")%>
</p>
...
<form name="importcat" action="<%= service.makeFormActionLink("") %>" method="POST">
	...
	<input type="hidden" name="<%= service.makeParameterName(CATConstants.REQUEST_PARAM_PAGE) %>" value="<%= CATConstants.JSP_PAGE_CAT_IMPORT_DIR_2 %>" />
	<tr>
		<td>
			<input type="text" size="50" name="<%= service.makeParameterName(CATConstants.REQUEST_PARAM_IMPORT_DIRECTORY) %>" value="" />
		</td>
	</tr>
	<tr>
		<td align="right">
			...
			&nbsp;
			<a class="textButton" href="javascript:document.importcat.submit();" title="Submit">
				<%=rb.getString("categoryImportDir1.submit")%>
			</a>
		</td>
	</tr>
</form>
